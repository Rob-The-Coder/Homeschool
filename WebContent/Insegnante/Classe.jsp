...
String Classe="";
if(request.getSession().getAttribute("Classe")!=null){
  Classe=(String) request.getSession().getAttribute("Classe");
} //endif
ArrayList<StudenteBean> Studenti=new ArrayList<StudenteBean>();
if(request.getSession().getAttribute("VetStudenti")!=null){
  Studenti=(ArrayList<StudenteBean>) request.getSession().getAttribute("VetStudenti");
} //endif
ArrayList<ArrayList<Integer>> VotiStudente=new ArrayList<ArrayList<Integer>>();
if(request.getSession().getAttribute("VotiStudente")!=null){
  VotiStudente=(ArrayList<ArrayList<Integer>>) request.getSession().getAttribute("VotiStudente");
} //endif
...
<html>
  ...
  <body>
    ...
    <div class="container-fluid ContenutoPagina">
      <div class="row mb-3">
        <div class="col">
          <p class="TestoNormale font-weight-bold">CLASSE <%= Classe %></p>
        </div>
      </div>
      <!-- CARTA PER LA ClASSE -->
      <% for(int i=0; i<Studenti.size(); i=i+1){ %>
        <div class="card mb-3" id="CARD">
	        <div class="card-body">
	          <div class="row">
	            <div class="col">
	              <p class="TestoNormale font-weight-bold h4"><%= Studenti.get(i).getCognome().toUpperCase() %> <%= Studenti.get(i).getNome().toUpperCase() %></p>               
	            </div>
	          </div>
	          <div class="row">
	            <div class="col">
	              <canvas id="GraficoStudente<%= i+1 %>" width="auto" height="50vh"></canvas>           
	            </div>
	          </div>    
	        </div>
	      </div>
    ...
    <script type="text/javascript">
      <% for(int i=0; i<Studenti.size(); i=i+1){ %>
	      var Questionari=[];
	      <% for(int j=0; j<VotiStudente.get(i).size(); j=j+1){ %>
	        Questionari.push("Q<%= j+1 %>");
	      <% }//endfor %>
        var ctx=document.getElementById("GraficoStudente<%= i+1 %>").getContext('2d');
        var myChart=new Chart(ctx, {
            type: 'line',
            data: {
              labels: Questionari,
              datasets: [{
                label: 'Voto del questionario',
                data: <%= VotiStudente.get(i) %>,
